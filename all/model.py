...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.userinerate = RESMLP(
            input_dim=userdim, hid_dim=args.hidden_units, dropout=args.dropout_rate, num_blocks=0
        )
        self.iteminerate = RESMLP(
            input_dim=itemdim, hid_dim=args.hidden_units, dropout=args.dropout_rate, num_blocks=0
        )
    ...
    def feat2emb(self, seq, feature_array, mask=None, include_user=False):
        ...
        all_item_emb = self.iteminerate(all_item_emb)
        if include_user:
            all_user_emb = torch.cat(user_feat_list, dim=2)
            all_user_emb = self.userinerate(all_user_emb)
